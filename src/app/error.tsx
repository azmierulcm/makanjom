...
export default function GlobalError({
  ...
  useEffect(() => {
    ...
    console.error('[Makanjom] Unhandled route error:', error);
  }, [error]);
  ...
  return (
    <div className="min-h-screen bg-[#faf9f7] flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md rounded-[3rem] border border-neutral-200 bg-white p-10 text-center shadow-2xl">
        ...
        <h1 className="text-2xl font-black tracking-tight text-neutral-950">Something went wrong</h1>
        <p className="mt-3 text-sm font-medium leading-relaxed text-neutral-500">
          An unexpected error occurred. Our team has been notified.
        </p>
        {error?.digest && (
          <p className="mt-2 text-[10px] font-mono text-neutral-300">ref: {error.digest}</p>
        )}

        <div className="mt-8 flex flex-col gap-3">
          <button
            onClick={reset}
            className="flex items-center justify-center gap-2 rounded-full bg-neutral-950 py-4 text-sm font-black uppercase tracking-widest text-white hover:bg-neutral-800"
          >
